...
public class Main {
                ...
                switch (res) {
                    ...
                    case "2":
                        ...
                        for (Tweet tweet : tweets) {
                            System.out.println("Tweet Id :" + tweet.getId() + "\t" + " And Content :" + "\t" + tweet.getContent());
                        }
    ...
    public static void showAllTweets() {
        ...
        if (tweets.isEmpty()) {
            throw new BusinessException(MassageException.tweetsListIsEmpty);
        }
        for (Tweet tweet : tweets) {
            String display = userService.getInfo(tweet.getUserId());
            System.out.println("Id  " + tweet.getId() + " : " + " name : " + display + " *** " + "post : " + tweet.getContent());
        }
    }
    ...
    public static void retweet(User user) {
        ...
        for (Tweet tweet : tweets) {
            if (tweet.getId().equals(tweetId)) {
                ...
                String newRetweet = retweet + " " + content;
                List<Tag> tags = junction.getTags(tweetId);
                tweetService.createNewTweet(newRetweet, tags, user);
    ...
    public static void editTweet(List<Tweet> tweets) {
        ...
        Tweet tweet = findTweetById(tweets, tweetId);
        ...
        if (res.equals("1")) {
            ...
            tweetService.updateContentByTweetId(newComment, tweet.getId());
            System.out.println("Content updated successfully");
        } else if (res.equals("2")) {
            List<Tag> tags = junction.getTags(tweet.getId());
            if (tags.isEmpty()) {
                List<Tag> chosenTags = chooseTags();
                ...
                if (finalT.equals("1")) {
                    ...
                    junction.delete(tweet.getId(), tagId);
                    System.out.println("tag deleted successfully");

                }
                if (finalT.equals("2")) {
                    List<Tag> chosenTags = chooseTags();
                    saveListTags(chosenTags, tweet.getId());
                }
            }
        } else {
            throw new BusinessException(MassageException.wrongNumber);
    ...
    private static Tweet findTweetById(List<Tweet> tweets, Long tweetId) {
        Tweet tweet;
        for (int i = 0; i < tweets.size(); i++) {
